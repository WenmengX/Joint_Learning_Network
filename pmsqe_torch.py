...
class PMSQE(nn.Module):
    ...
    def __init__(self, window_name = 'sqrt_hann', window_weight = 1.0, bark_eq = True, gain_eq = True):
        ...
        mask_sll = np.zeros(shape=[self.nbins // 2 + 1])
    ...
    def forward(self, output, target, pad_mask,alpha):
        """
        ...
        # SLL equalization
        ref_spectra = self.magnitude_at_standard_listening_level(target, pad_mask)   # (1,600,257)
        deg_spectra = self.magnitude_at_standard_listening_level(output, pad_mask)

        # Bark spectra computation
        ref_bark_spectra = self.bark_computation(ref_spectra)  # (1,600,49)
        ...
        D, DA = self.compute_distortion_tensors(ref_bark_spectra, deg_bark_spectra)  # (1,600,49)

        # Per-frame distortion
        audible_power_ref = self.compute_audible_power(ref_bark_spectra, 1.0)   # (1,600,1)
        wD_frame, wDA_frame = self.per_frame_distortion(D, DA, audible_power_ref)  # (1,600,1)
