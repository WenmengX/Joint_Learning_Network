...
from scipy.signal import fftconvolve,correlate,decimate
...
class AcousticScene:
    ...
    def simulate(self):
        ...
        if self.T60 == 0:
            ...
            Tdiff = 12 / 60.0 * self.T60
            Tmax = 40 / 60.0 * self.T60
            if self.T60 < 0.15: Tdiff = Tmax # Avoid issues with too short RIRs
            nb_img2 = 2 * Tdiff / (np.array(self.room_sz) / 343.0)
            nb_img = [int(n) for n in np.ceil(nb_img2)]
        ...
        '''
        mic_signals_sources = np.zeros([len(self.t),self.mic_pos.shape[0]])
        dp_mic_signals_sources = np.zeros([len(self.t), self.mic_pos.shape[0]])
        for source_idx in range(num_source):
            ...
            '''
            ####
            # room = pra.ShoeBox(self.room_sz, self.fs, materials=pra.Material(0.2), max_order=int(np.mean(nb_img)))  # material 0.2 for 500ms
            room = pra.ShoeBox(self.room_sz, self.fs, materials=pra.Material(random.randint(20, 100.0)/100), max_order=3)  # material 0.2 for 500ms, 0.02-->1, 1.0-->0
            room.add_microphone_array(self.mic_pos.T)
            signal_segs = self.source_signal[:,source_idx]
            room.add_source(self.traj_pts[0,:,source_idx], signal=signal_segs)
            room.compute_rir()
            room.simulate()
            mic_signals = room.mic_array.signals.T
            mic_signals = mic_signals[0:len(self.t), :]

            dp_room = pra.ShoeBox(self.room_sz, self.fs, materials=pra.Material(1.0),max_order=1)  # material 0.2 for 500ms, 0.02-->1, 1.0-->0
            dp_room.add_microphone_array(self.mic_pos.T)
            dp_room.add_source(self.traj_pts[0,:,source_idx], signal=signal_segs)
            dp_room.compute_rir()
            dp_room.simulate()
            dp_mic_signals = dp_room.mic_array.signals.T
            dp_mic_signals = dp_mic_signals[0:len(self.t), :]
            #####

            #RIRs_sources += [RIRs]
            mic_signals_sources += mic_signals
            #dp_RIRs_sources += [dp_RIRs]
            dp_mic_signals_sources += dp_mic_signals
        ...
        if self.noise_signal is None:
            self.noise_signal = np.random.standard_normal(mic_signals_sources.shape)
        #mic_signals = np.sum(mic_signals_sources, axis=2) # (nsamples, nch)
        #dp_mic_signals = np.sum(dp_mic_signals_sources, axis=2)
        ac_pow = np.mean([acoustic_power(dp_mic_signals_sources[:,i]) for i in range(dp_mic_signals_sources.shape[1])])
        ...
        mic_signals_sources += noise_signal[0:len(self.t), :]
        ...
        if hasattr(self, 'source_vad'):
            ...
            for source_idx in range(num_source):
                ...
                '''
                vad_room = pra.ShoeBox(self.room_sz, self.fs, materials=pra.Material(1.0),max_order=1)  # material 0.2 for 500ms, 0.02-->1, 1.0-->0
                vad_room.add_microphone_array(self.mic_pos.T)
                vad_signal_segs = self.source_vad[:,source_idx]
                vad_room.add_source(self.traj_pts[0, :, source_idx], signal=vad_signal_segs)
                vad_room.compute_rir()
                vad_room.simulate()
                vad_mic_signals = vad_room.mic_array.signals.T
                vad = vad_mic_signals[0:len(self.t), :]
                vad_sources = vad[0:len(self.t), :].mean(axis=1) > vad[0:len(self.t), :].max() * 1e-3
                self.mic_vad_sources += [vad_sources]  # binary value
                #####
        ...
        return mic_signals_sources, dp_mic_signals_sources

    def add_noise(self, dp_mic_signals_sources, mic_signal_r):
        num_source = self.traj_pts.shape[-1]
        if self.noise_signal is None:
            self.noise_signal = np.random.standard_normal(mic_signal_r.shape)

        ac_pow = np.mean([acoustic_power(dp_mic_signals_sources[:, i]) for i in range(dp_mic_signals_sources.shape[1])])
        ac_pow_noise = np.mean([acoustic_power(self.noise_signal[:, i]) for i in range(self.noise_signal.shape[1])])
        noise_signal = np.sqrt(ac_pow / 10 ** (self.SNR / 10)) / np.sqrt(ac_pow_noise) * self.noise_signal
        mic_signal_r += noise_signal[0:len(self.t), :]
        mic_signals_sources = mic_signal_r
        if hasattr(self, 'source_vad'):
            self.mic_vad_sources = []  # for vad of separate sensor signals of source
            for source_idx in range(num_source):
                vad_room = pra.ShoeBox(self.room_sz, self.fs, materials=pra.Material(1.0),
                                       max_order=1)  # material 0.2 for 500ms, 0.02-->1, 1.0-->0
                vad_room.add_microphone_array(self.mic_pos.T)
                vad_signal_segs = self.source_vad[:, source_idx]
                vad_room.add_source(self.traj_pts[0, :, source_idx], signal=vad_signal_segs)
                vad_room.compute_rir()
            ...
            self.mic_vad_sources = np.array(self.mic_vad_sources).transpose(1, 0)  # (nsample, nsources)
            self.mic_vad = np.sum(self.mic_vad_sources,
                                  axis=1) > 0.5  # for vad of mixed sensor signals of sources (nsample)

        return mic_signals_sources

class MixedDataset(Dataset):
    def __init__(self, dataset_1, dataset_2, ratio_1_to_2=14):
        """
        Args:
            dataset_1: 第一个数据集（主数据集，14条/组）
            dataset_2: 第二个数据集（辅数据集，1条/组）
            ratio_1_to_2: 混合比例（默认14:1）
        """
        self.dataset_1 = dataset_1
        self.dataset_2 = dataset_2
        self.ratio = ratio_1_to_2

        # 计算总组数（以 dataset_2 的长度为基准）
        self.num_groups = min(len(dataset_2), len(dataset_1) // ratio_1_to_2)

        # 总样本数 = 每组 (ratio+1) 条 * 组数
        self.total_length = self.num_groups * (ratio_1_to_2 + 1)
    ...
    def __getitem__(self, idx):
        # 计算当前样本属于哪个组
        group_idx = idx // (self.ratio + 1)
        pos_in_group = idx % (self.ratio + 1)

        if pos_in_group < self.ratio:
            # 从 dataset_1 取数据（确保不越界）
            data_1_idx = group_idx * self.ratio + pos_in_group
            return self.dataset_1[data_1_idx % len(self.dataset_1)]
        else:
            # 从 dataset_2 取数据
            return self.dataset_2[group_idx % len(self.dataset_2)]

class CleanDataset(Dataset):
    """ Dataset with random DNS3 utterances.
    You need to indicate the path to the root of the DNS3 dataset in your file system
    and the length of the utterances in seconds.
    """

    def __init__(self, path, T, fs, num_source, start_time, utter_num, size=None, return_vad=False, clean_silence=True):
        self.path = path
        self.T = T
        self.fs = fs
        ...
        self.file_list = [os.path.join(path, f) for f in os.listdir(path) if f.endswith('.wav')]
        self.file_list = self.file_list[:utter_num]
    ...
    def __getitem__(self, idx):
        ...
        while idx >= len(self.file_list): idx -= len(self.file_list)
        ...
        for source_idx in range(self.num_source):
            ...
            s = s[int(self.start_time * self.fs): int((self.T + self.start_time) * self.fs)]
            s -= s.mean()
...
class NoiseDataset():
    ...
    def get_random_noise(self, mic_pos=None):
        ...
        if noise_type == 'spatial_white':
            ...
            while nsample < nsample_desired:
                noise_copy = np.concatenate((noise_copy, noise), axis=0)
                nsample = noise_copy.shape[0]
            max_segment_attempts = 16000 * 30
            segment_attempts = 0
            while segment_attempts < max_segment_attempts:

                    st = random.randint(0, noise_copy.shape[0] - nsample_desired)
                    ed = st + nsample_desired
                    noise_signal = noise_copy[st:ed, :]

                    if np.any(noise_signal != 0):
                        # print("找到满足条件的噪声信号片段")
                        break
                    segment_attempts += 1
    ...
    def gen_diffuse_noise(self, noise, T, fs, mic_pos, nfft=256, c=343.0, type_nf='spherical'):
        ...
        ww = 2*np.pi*self.fs*np.array([i for i in range(nfft//2+1)])/nfft
            ...
            for q in range(0,M):
                if p == q:
                    ...
                    if type_nf == 'spherical':
                        DC[p,q,:] = np.sinc(ww*dist/(c*np.pi))
...
class FixTrajectoryDataset(Dataset):
    ...
    def __init__(self, data_dir, dataset_sz, transforms=None, return_acoustic_scene=False):
        ...
        self.data_paths = [0]*dataset_sz
        self.dp_data_paths = [0]*dataset_sz
        data_names = os.listdir(data_dir)
        for fname in data_names:
            front, ext = os.path.splitext(fname)
            if ext == '.wav':
                if front[0:2] == 'dp':
                    #self.dp_data_paths.append((os.path.join(data_dir, fname)))
                    self.dp_data_paths[int(front[2:])] = os.path.join(data_dir, fname)
                else:
                    #self.data_paths.append((os.path.join(data_dir, fname)))
                    self.data_paths[int(front)] = os.path.join(data_dir, fname)
    ...
    def __getitem__(self, idx):
        ...
        dp_sig_path = self.dp_data_paths[idx]
        ...
        mic_signals, dp_mic_signals, acoustic_scene = load_file(acoustic_scene, sig_path, dp_sig_path, acous_path)
...
class Segmenting_SRPDNN(object):
    ...
    def __call__(self, x, acoustic_scene):
        ...
        acoustic_scene.DOAw = []
        for source_idx in range(num_source):
            DOAw = np.lib.stride_tricks.as_strided(DOA[:,:,source_idx], shape=shape_DOAw, strides=strides_DOAw) # segment the matrix in shape(shape_DOAw)
            DOAw = np.ascontiguousarray(DOAw)
            for i in np.flatnonzero(np.abs(np.diff(DOAw[..., 1], axis=1)).max(axis=1) > np.pi): # diff: find difference between adjacent elements; flatnonzero: find indices of nonzero elements
...
class RandomTrajectoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mic_signals, dp_mic_signals = acoustic_scene.simulate()
        if self.transforms is not None:
            for t in self.transforms:
                mic_signals, acoustic_scene = t(mic_signals, acoustic_scene)
    ...
    def beta_SabineEstimation(self, room_sz, T60, abs_weights=[1.0] * 6):
        ...
        result = opt.minimize(t60error, 0.5, args=(T60, room_sz, abs_weights), bounds=[[0, 1]])
    ...
    def getRandomScene(self, idx):
        ...
        beta = self.beta_SabineEstimation(room_sz, T60, abs_weights)
...
class S3A_Vislab_RandomTrajectoryDataset(Dataset):
    """ Dataset Acoustic Scenes with random trajectories.
    The length of the dataset is the length of the source signals dataset.
    """

    def __init__(self, sourceDataset, num_source, source_state, room_sz, T60, abs_weights, array_setup,
                 array_pos, noiseDataset, SNR, nb_points, min_dis, c=343.0, transforms=None):
        """
        sourceDataset: dataset with the source signals (such as LibriSpeechDataset)
        num_source: Number of sources
        source_state: Static or mobile sources
        ...
        self.N = array_setup.mic_pos.shape[0]
        self.array_pos = array_pos if type(array_pos) is Parameter else Parameter(array_pos)
        self.mic_scale = array_setup.mic_scale if type(array_setup.mic_scale) is Parameter else Parameter(
            array_setup.mic_scale)
        self.min_dis = min_dis if type(min_dis) is Parameter else Parameter(min_dis)
        self.noiseDataset = noiseDataset
        self.SNR = SNR if type(SNR) is Parameter else Parameter(SNR)
        self.nb_points = nb_points
    ...
    def __getitem__(self, idx):
        if idx < 0: idx = len(self) + idx
        acoustic_scene, random_index = self.getRandomScene(idx)
        src_signal = acoustic_scene.source_signal.squeeze()
        rir_data = scipy.io.loadmat('subset_IR_vislab.mat')
        rirs = rir_data['subset_IR']  # (12,2,n_samples_rir)
        down_rirs = self.downsample_rirs(rirs)
        h1 = down_rirs[random_index, :, 0].flatten()
        h2 = down_rirs[random_index, :, 1].flatten()
        index_h1 = np.argmax(h1)
        index_h2 = np.argmax(h2)
        conv1 = fftconvolve(src_signal, h1[index_h1:], mode='full')[:64000]
        conv2 = fftconvolve(src_signal, h2[index_h2:], mode='full')[:64000]
        stereo_audio = np.vstack([conv1, conv2])
        mic_signals_r = stereo_audio.T

        dp_conv1 = fftconvolve(src_signal, h1[index_h1:index_h1 + 160], mode='full')[:64000]
        dp_conv2 = fftconvolve(src_signal, h2[index_h2:index_h1 + 160], mode='full')[:64000]
        dp_stereo_audio = np.vstack([dp_conv1, dp_conv2])
        dp_mic_signals = dp_stereo_audio.T
        mic_signals = acoustic_scene.add_noise(dp_mic_signals, mic_signals_r)
    ...
    def align_src_signals(self, src_signal, delay1, delay2):
        """
        根据两个时延将单通道信号转换为两通道信号。

        :param src_signal: 单通道源信号，类型为 numpy 数组
        :param delay1: 第一个通道的时延，整数
        :param delay2: 第二个通道的时延，整数
        :return: 两通道信号，形状为 (信号长度, 2) 的 numpy 数组
        """
        signal_length = len(src_signal)
        # 初始化两通道信号数组
        dp_mic_signals_new = np.zeros((signal_length, 2))

        # 处理第一个通道的时延
        if delay1 > 0:
            # 右移信号，前面补零
            dp_mic_signals_new[delay1:, 0] = src_signal[:-delay1]
        elif delay1 < 0:
            # 左移信号，后面补零
            dp_mic_signals_new[:delay1, 0] = src_signal[-delay1:]
        else:
            # 无时延，直接复制信号
            dp_mic_signals_new[:, 0] = src_signal

        # 处理第二个通道的时延
        if delay2 > 0:
            # 右移信号，前面补零
            dp_mic_signals_new[delay2:, 1] = src_signal[:-delay2]
        elif delay2 < 0:
            # 左移信号，后面补零
            dp_mic_signals_new[:delay2, 1] = src_signal[-delay2:]
        else:
            # 无时延，直接复制信号
            dp_mic_signals_new[:, 1] = src_signal

        return dp_mic_signals_new

    def downsample_rirs(self, rirs_original: np.ndarray,
                        original_fs: int = 48000,
                        target_fs: int = 16000) -> np.ndarray:

        assert original_fs > target_fs, "目标采样率必须小于原始采样率"
        assert original_fs % target_fs == 0, "采样率必须为整数倍关系"

        # 计算降采样因子
        q = original_fs // target_fs

        rirs = rirs_original.transpose(0, 2, 1)  # 新形状：(n_pos, n_samples, n_chn)
        test_signal = rirs[0, :, 0]
        n_samples_new = len(decimate(test_signal, q=q, ftype='fir'))

        # 初始化输出数组
        n_positions, _, n_channels = rirs.shape
        downsampled_rirs = np.zeros((n_positions, n_samples_new, n_channels))

        # 并行处理所有位置和通道
        for pos in range(n_positions):
            for chn in range(n_channels):
                signal = rirs[pos, :, chn]
                downsampled_rirs[pos, :, chn] = decimate(signal, q=q, ftype='fir')

        return downsampled_rirs
    ...
    # new added
    def beta_SabineEstimation(self, room_sz, T60, abs_weights=None):
    ...
    def getRandomScene(self, idx):
        ...
        # Microphones
        array_pos = np.array((4.19, 3.95, 1.62))
        mic_scale = self.mic_scale.getValue()
        mic_pos = array_pos + self.array_setup.mic_pos * mic_scale
        array_pos[1] = mic_pos[0][1]
        # Noises
        ...
        for source_idx in range(num_source):
            ...
            relative_coordinate = np.array((4.19, 3.95, 1.62))
            new_coordinates = known_coordinates + relative_coordinate
            selected_coordinate = new_coordinates[random_index]
            trajectory[:, :, source_idx] = selected_coordinate
            traj_pts[0, :, source_idx] = selected_coordinate
            DOA[:, :, source_idx] = cart2sph(trajectory[:, :, source_idx] - array_pos)[:, 1:3]
        acoustic_scene = AcousticScene(
            room_sz=room_sz,
            T60=T60,
        ...
        return acoustic_scene, random_index