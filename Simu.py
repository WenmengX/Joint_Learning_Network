...
from Dataset_ori import Parameter
import Dataset_ori as at_dataset
import tqdm
if args.train:
    #data_num = 166816
	data_num = 20000
...
for idx in tqdm.tqdm(range(data_num)):
	mic_signals, dp_mic_signals, acoustic_scene = dataset[idx]
	sig_path = save_dir + '/' + str(idx) + '.wav'
	dp_sig_path = save_dir + '/' + 'dp' + str(idx) + '.wav'
	acous_path = save_dir + '/' + str(idx) + '.npz'
	save_file(mic_signals, dp_mic_signals, acoustic_scene, sig_path, dp_sig_path, acous_path)
