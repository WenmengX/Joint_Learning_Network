...
def WB_PESQ(ref, est, sr=16000):
    if sr != 16000:
        wb_ref = librosa.resample(ref, orig_sr=sr, target_sr=16000)
        wb_est = librosa.resample(est, orig_sr=sr, target_sr=16000)
    else:
        wb_ref = ref
        wb_est = est
...
def PY_WB_PESQ(ref, est, sr=16000):
    if sr != 16000:
        nb_ref = librosa.resample(ref, orig_sr=sr, target_sr=16000)
        nb_est = librosa.resample(est, orig_sr=sr, target_sr=16000)
    else:
        nb_ref = ref
        nb_est = est
    # nb_pesq downsample to 8000 internally.
    return nb_pesq(nb_ref, nb_est, 16000)
def NB_PESQ(ref, est, sr=16000):
    if sr != 8000:
        nb_ref = librosa.resample(ref, orig_sr=sr, target_sr=8000)
        nb_est = librosa.resample(est, orig_sr=sr, target_sr=8000)
    else:
        nb_ref = ref
        nb_est = est
    # nb_pesq downsample to 8000 internally.
...
def PY_NB_PESQ(ref, est, sr=16000):
    if sr != 8000:
        nb_ref = librosa.resample(ref, orig_sr=sr, target_sr=8000)
        nb_est = librosa.resample(est, orig_sr=sr, target_sr=8000)
    else:
