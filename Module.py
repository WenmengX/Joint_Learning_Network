...
class getMetric(nn.Module):
	...
	def angular_error(self, est, gt, ae_mode):
		...
		"""
		if ae_mode == 'azi':
			ae = torch.abs((est-gt+180)%360 - 180)  # 将差值映射在-180~180取绝对值
		elif ae_mode == 'ele':
			ae = torch.abs(est-gt)  # -90~90 取绝对值
		elif ae_mode == 'aziele':
			ele_gt = gt[0, ...].float() / 180 * np.pi
			azi_gt = gt[1, ...].float() / 180 * np.pi
			ele_est = est[0, ...].float() / 180 * np.pi
			azi_est = est[1, ...].float() / 180 * np.pi
			aux = torch.cos(ele_gt) * torch.cos(ele_est) + torch.sin(ele_gt) * torch.sin(ele_est) * torch.cos(azi_gt - azi_est)   # 真实方向和估计方向之间的夹角的余弦值
...
class DPIPD(nn.Module):
	...
	def \
			__init__(self, ndoa_candidate, mic_location, nf=257, fre_max=8000, ch_mode='M', speed=343.0):
...
class SourceDetectLocalize(nn.Module):
	...
	def forward(self, pred_ipd, dpipd_template, doa_candidate):
		...
		pred_ss = torch.bmm(pred_ipd.contiguous().view(nb, nt, -1), dpipd_template.contiguous().view(nb, nele, nazi, -1).permute(0, 3, 1, 2).view(nb, nmic * nf, -1))/(nmic*nf/2)  # (nb, nt, nele*nazi)
		...
		if self.meth_mode == 'IDL': # iterative source detection and localization
			...
			for source_idx in range(self.max_num_sources):
				map = torch.bmm(pred_ipd.contiguous().view(nb, nt, -1),dpipd_template.contiguous().view(nb, nele, nazi, -1).permute(0, 3, 1, 2).view(nb, nmic * nf, -1)) / (nmic * nf / 2)  # (nb, nt, nele*nazi)
				...
				pred_DOA = np.stack((ele_candidate[ele_max_idx], azi_candidate[azi_max_idx]),axis=-1)  # (nb, nt, 2)
				...
				for b_idx in range(nb):
					for t_idx in range(nt):
						max_dpipd_template[b_idx, t_idx, :, :] = dpipd_template[b_idx, ele_max_idx[b_idx, t_idx], azi_max_idx[b_idx, t_idx], :, :] * 1.0  # (nb, nt, 2nf, nmic-1)
