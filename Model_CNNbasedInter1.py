...
class Joint_Learning(nn.Module):
    ...
    def __init__(self, input_size=4,
        ...
        self.cnnT_deep = 3
        self.convT0 = nn.ConvTranspose2d(in_channels = self.hidden_size, out_channels = self.hidden_size//2, kernel_size= (5,5), stride= (2,2), output_padding = (1,1))
        self.bnT0 = nn.BatchNorm2d(self.hidden_size // 2)
        self.T_prelu0 = nn.PReLU(self.hidden_size//2)
        self.convT1 = nn.ConvTranspose2d(in_channels=self.hidden_size, out_channels=self.hidden_size //4, kernel_size= (5,5), stride= (2,2), output_padding = (0,1))
        self.bnT1 = nn.BatchNorm2d(self.hidden_size // 4)
        self.T_prelu1 = nn.PReLU(self.hidden_size //4)
        self.convT2 = nn.ConvTranspose2d(in_channels= 63*2, out_channels=self.input_size * 2, kernel_size= (5,5), stride= (2,2), output_padding = (0,1))  # output_padding = (0,1) for Librispeech   (0,0) for DNS3
        self.bnT2 = nn.BatchNorm2d(self.input_size * 2)
        self.T_prelu2 = nn.PReLU(self.input_size * 2)
    ...
    def forward(self, data_x, eps = 1e-6, return_attn_score=False):
        ...
        Doa2Se2T = self.T_prelu0(self.convT0(Doa2Se3))
        Doa2Se1T = self.T_prelu1(self.convT1(torch.cat((Doa2Se2T, Doa2Se2), dim=1))) + Doa2Se1
        Doa2Se0T = self.T_prelu2(self.convT2(torch.cat((Doa2Se1T, Doa2Se1), dim=1))) + Doa2Se0
