...
class Joint_Learning(nn.Module):
    ...
    def __init__(self, input_size=4,
        ...
        self.LstmDoa2Se = nn.LSTM(input_size=self.input_size * 2, hidden_size=self.hidden_size, batch_first=True, bidirectional=True)
        self.LstmDoa2Se.flatten_parameters()
        self.LstmSe2Doa = nn.LSTM(input_size=self.hidden_size, hidden_size=self.hidden_size // 2, batch_first=True, bidirectional=True)
        self.doa2se = nn.Linear(self.hidden_size*2,4)
    ...
    def forward(self, data_x, eps = 1e-6, return_attn_score=False):
        ...
        # SE ############################################################### ipd0: (257,24,4)
        # xJoint (257,300,254)
        Doa2Se, _ = self.LstmDoa2Se(torch.cat((x_RIRI, self.tanh(self.hidden2ch(xJoint)).reshape(nb,nf,nt,nc)), dim = 3).reshape(nb*nf,nt,nc*2))
        x_RIRI = self.prelu(self.doa2se(Doa2Se)) + x_RIRI
        del Doa2Se
