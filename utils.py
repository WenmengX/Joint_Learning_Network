...
def cv_istft(features, n_fft, hop_length, win_length, length=None, use_mag_phase=False):
    """
...
def save_file(mic_signal, dp_mic_signal, acoustic_scene, sig_path, dp_sig_path, acous_path):
    
    if sig_path is not None:
        soundfile.write(sig_path, mic_signal, acoustic_scene.fs)
        soundfile.write(dp_sig_path, dp_mic_signal, acoustic_scene.fs)
...
def load_file(acoustic_scene, sig_path, dp_sig_path, acous_path):

    if sig_path is not None:
        mic_signal, fs = soundfile.read(sig_path)
        dp_mic_signal, fs = soundfile.read(dp_sig_path)
    ...
    if (sig_path is not None) & (acous_path is not None):
        return mic_signal, dp_mic_signal, acoustic_scene
    elif (sig_path is not None) & (acous_path is None):
        return mic_signal, dp_mic_signal
